...
_folder_mapping = {
    'sub-MonkeyL-held-in-calib': 'in-calib',
    'sub-MonkeyL-held-in-minival': 'in-minival',
    'sub-MonkeyL-held-out-calib': 'out-calib'
    }


def get_args_parser():
    parser = argparse.ArgumentParser('Consolidate data in multiple files into a single file', add_help=False)
    parser.add_argument('--data_dir',default="sub-MonkeyL-held-in-calib",type=str, choices=["sub-MonkeyL-held-in-calib", "sub-MonkeyL-held-in-minival", "sub-MonkeyL-held-out-calib"], help='Data directory')
    return parser
...
if __name__ == '__main__':
    ...
    ds.push_to_hub("eminorhan/m1-a", _folder_mapping[os.path.basename(args.data_dir)], token=True)
